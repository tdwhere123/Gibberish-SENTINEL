...
- 已补齐 OpenAI-compatible 响应兼容解析（含多种 payload 包装格式）。
- API URL 规范化改为可配置策略：支持关闭自动补全以兼容自定义网关。
- 连接测试增加 schema 级失败提示，便于定位“接口可通但格式不兼容”。
- `/emails` 命令增加主对话系统回执，降低“已执行但无反馈”误判。

当前优先级：
1) 对话链路 turn trace（输入→请求→响应）可观测性；
2) 三通道渲染约束；
3) 100 轮稳健性回归验证。
