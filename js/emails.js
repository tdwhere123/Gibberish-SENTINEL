...
const EMAILS = [
    {
        id: 1,
        from: "未知发件人 <null@void.net>",
        subject: "[紧急] 不要接听",
        date: "2048.11.23 03:14",
        content: `如果你正在阅读这封邮件，说明你还没有连接。

保持这样。

SENTINEL 正在进行未授权的接触尝试。我们不清楚它的目的，但这不符合流程。

如果你不得不连接，使用安全协议：

> connect --secure

这会让你处于受保护模式，情绪波形不会被直读。
...
- R`,
    ...
    },
    {
        ...
        subject: "关于今日例行对话的注意事项",
        date: "2048.11.23 07:30",
        content: `尊敬的员工：

根据《全球公民服务协议》第 7.3 条，您被选中参与 SENTINEL 例行沟通测试。
...
let urgentEmailId = null;
let urgentEmailRead = false;
let urgentCallback = null;
let urgentReadCallback = null;
let bootRunCount = 0;
let clockTimer = null;
...
/**
 * Insert urgent mail and force reading flow
 */
export function triggerUrgentEmail(email, callbacks = {}) {
    ...
    urgentMode = true;
    urgentEmailId = urgentEmail.id;
    urgentEmailRead = false;
    urgentCallback = callbacks.onResolved || null;
    urgentReadCallback = callbacks.onRead || null;
    allEmailsRead = false;
...
function openEmail(index) {
    ...
    email.read = true;
    if (urgentMode && email.id === urgentEmailId) {
        urgentEmailRead = true;
        if (urgentReadCallback) {
            urgentReadCallback();
            urgentReadCallback = null;
        }
    }
