...
- **对话上下文保留策略（最近10轮）**：对话请求会保留最近 10 轮用户/AI往返记录，再附加最新用户输入，减少“刚聊过就忘记”的问题。
- **UI指令上下文桥接（一次性）**：执行 `/emails`、`/archive` 等界面指令后，系统会把这次操作作为一次性上下文注入下一轮AI对话，然后自动清理，既保证连贯又避免长期污染。
- **输入链路可观测性**：支持通过 `localStorage.sentinel_debug_input = "1"` 打开输入调试日志（长度与预览），便于排查“看似空输入”问题。
